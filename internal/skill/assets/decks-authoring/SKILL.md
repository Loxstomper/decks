...
description: >
  ...
  decks project. Covers creating/modifying deck.html files,
...
# decks-authoring skill

You are editing reveal.js decks managed by decks. This file teaches
you every rule you need to author valid decks that the editor, the presenter,
and the offline renderer all handle correctly.

Read `AUTHORING.md` (installed beside this file) for the full contracts
reference; this skill is the practical how-to guide with annotated snippets.
...
```
<workspace>/          # any directory containing decks/ — the `decks` binary lives on $PATH
...
stylesheet (`assets/vendor/decks-layout.css`).
...
- `data-chart-data` must be **parseable JSON** (a `{type, data, options?}`
  config). `decks validate` rejects malformed JSON.
...
- `data-qr` must be a **non-empty** string (the payload). `decks validate`
...
- reveal.js and all plugins: already vendored in `assets/vendor/` by
  `decks new`. Do not modify these files.
...
`decks` lives on `$PATH` and finds the workspace itself, so you can run it from
anywhere — including from inside `decks/<name>/`. It resolves the root as
`--dir <path>` (before the subcommand) › `$DECKS_DIR` › the nearest ancestor
containing `decks/`, and prints the root it chose. If a command reports

```
error: /some/dir is not a decks workspace (no decks/).
```

you are outside a workspace: pass `decks --dir <path> …`, don't `cd` and retry
blindly. **`decks new` is the only command that creates a workspace** — every
...
inside `decks/my-talk/`, `decks validate assets` looks for a deck called
`assets`, not the deck you are standing in; write `decks validate .` for that.

ALWAYS run `decks validate` after editing a deck — it exits non-zero with
readable diagnostics when the deck is malformed, so you can fix it before the
canvas or presenter silently breaks:

```bash
decks validate my-talk && echo "deck is valid"
```
...
See `AUTHORING.md` (beside this file) for the WHY behind each rule.
