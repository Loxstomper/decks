...
- [x] **P17-1 — Inline serializer + allowlist.** A pure `contenteditable` DOM → canonical inline-HTML serializer over a fixed allowlist (`strong`/`em`/`u`/`s`/`a[href,target,rel]`/`span[style: color,font-size]`); normalize browser junk (`<div>`/`<font>`/`&nbsp;`/style soup) to the allowlist. _Done when:_ a leaf with mixed marks round-trips byte-stable; off-allowlist input is normalized; unit-tested. (Spec 02, 03)
- [x] **P17-2 — Paste & input sanitization.** Strip scripts, event handlers, external resource URLs, and `javascript:` hrefs from contenteditable/paste before it reaches the model; localize pasted images via the existing asset pipeline. _Done when:_ pasting hostile/styled HTML yields only allowlisted marks; a pasted image is localized; offline-guard holds. (Spec 12, 08)
- [x] **P17-3 — Inline-aware writeback.** Replace the flatten-to-text `applyTextEditToModel` path with an inline-preserving `applyRichTextEdit(eid, html)` — serializes the leaf's inline DOM to allowlisted HTML; only the leaf's child subtree goes dirty. _Done when:_ committing an edit on a leaf containing `<strong>`/`<a>` preserves them; the element tag bytes and all siblings are byte-identical. (Spec 02)
- [x] **P17-4 — Model + validate recognize inline marks.** `classify` treats allowlisted marks as managed inline content *within* leaves (not new leaves, not passthrough); `validate.go` + `layout.ts` accept them; no `data-eid` is stamped on a mark. Keep allowed-sets synced. _Done when:_ `validate` passes a deck with inline marks; marks carry no eid. (Spec 02, 12)
- [x] **P17-5 — Round-trip corpus extension.** Add inline-mark + link fixtures to the golden byte-stability corpus (X-2). _Done when:_ the golden suite covers inline marks/links and stays byte-stable. (Spec 12)

### Lane B — Formatting UI (depends A)
- [x] **P17-6 — Range mark commands.** `toggleInline(tag)` wrap/unwrap over the live selection range for bold/italic/underline/strike + font-size + colour-run (range wrap, not `execCommand`); each one undo + one autosave, byte-stable. _Done when:_ selecting a word and toggling bold writes `<strong>` around just that range; toggling again removes it cleanly. (Spec 04, 02)
- [x] **P17-7 — Floating selection toolbar.** On text selection within an active in-place edit, show a toolbar at the selection (B/I/U/S, font size, text colour, link); Cmd/Ctrl+B/I/U shortcuts; dismiss on blur/Escape. _Done when:_ the toolbar appears on selection and each control applies its mark to the run. (Spec 04)
- [x] **P17-8 — Block-level text controls.** Inspector controls (whole-leaf): text-align (left/center/right/justify) + list indent/outdent. _Done when:_ align writes inline style on the leaf and indent/outdent re-nests the list; byte-stable + undoable. (Spec 04, 09)

### Lane C — Links (depends A)
- [x] **P17-9 — Link commands.** `linkRange` (wrap the selected range in `<a href>`) and `linkElement` (wrap a whole selected element), plus edit/remove; reject `javascript:` hrefs; external hrefs allowed (navigation, not a resource load). One undo + one autosave each, byte-stable. _Done when:_ linking a range/element writes `<a href>`, edit updates the href, remove unwraps; `javascript:` is refused. (Spec 04, 02, 12)
- [x] **P17-10 — Link editor UI + context menu.** An href popover (add/edit/remove) wired into the selection toolbar (P17-7) and the text-leaf context-menu actions. _Done when:_ both entry points add/edit/remove a link; the offline-guard is unaffected by an external href. (Spec 04)

### Lane D — Image alt text
- [x] **P17-11 — Alt-text inspector field.** Editable `alt` on a selected image; seed from the provider description on insert; carried through upload/drag/paste; empty `alt=""` allowed. _Done when:_ editing `alt` writes the attribute byte-stable + undoable; provider inserts seed `alt`. (Spec 08)

### Lane E — Command palette + shortcut help
- [x] **P17-12 — Command registry + palette.** A pure command registry (label/run/when) shared with menus; ⌘/Ctrl-K opens a searchable palette dispatching the **same `deckStore` commands**. _Done when:_ the palette opens, filters, and runs a command via the same path as the menu. (Spec 04)
- [x] **P17-13 — Shortcut help overlay.** `?` opens a cheat-sheet of the editor's existing shortcuts (nudge/undo/redo/delete/marquee/save/present), guarded against text-editing focus; Escape dismisses. _Done when:_ `?` shows the overlay listing current shortcuts and is not triggered mid-edit. (Spec 04)

### Lane F — Charts (Go + FE)
- [x] **P17-14 — Vendor Chart.js reveal plugin (offline).** `go:embed` the plugin into the binary, copy it into `decks/<name>/assets/vendor/` on `slides new`/`slides vendor`, enable it in the scaffold template; linked relatively. _Done when:_ a fresh deck links the chart plugin locally with zero external URLs. (Spec 03, 12, 01)
- [x] **P17-15 — Chart block + data editor.** Insert-palette chart block (type: bar/line/pie/…) writing `<canvas data-chart data-chart-data='…'>`; inspector JSON data/options editor. _Done when:_ inserting a chart renders it in the canvas; editing the data updates it; persists byte-stable. (Spec 03)
- [x] **P17-16 — Chart contract + thumbnail placeholder.** `validate.go` + model accept `data-chart`/`data-chart-data`; the thumbnail builder paints a chart placeholder (script-free). _Done when:_ `validate` passes a chart deck and the thumbnail shows a placeholder, no external URLs. (Spec 03, 06, 12)

### Lane G — Slide numbers + footers
- [x] **P17-17 — Slide-number toggle.** Deck-level control writing reveal's `slideNumber` (with a format option) into the deck's reveal init. _Done when:_ toggling shows numbers in edit + present; byte-stable. (Spec 09, 10)
- [x] **P17-18 — Deck footer (CSS-based).** Footer text + optional logo as a **managed `custom.css`** overlay rule; per-slide `data-footer-hidden` opt-out (recognized by `validate.go` + model). _Done when:_ setting a footer shows a fixed overlay across slides, a marked slide hides it, and it round-trips byte-stable. (Spec 09, 12)

### Lane H — Present-mode polish
- [x] **P17-19 — Vendor annotation + laser plugins (offline, ephemeral).** `go:embed` + per-deck copy of a chalkboard/annotation plugin and a laser pointer, enabled **only on the present route**; annotations never write to `deck.html`. _Done when:_ the present route loads both locally, drawing/laser work, `deck.html` is unchanged after use, zero external URLs. (Spec 10, 12)
- [x] **P17-20 — Auto-advance settings UI.** Per-deck default + per-slide `data-autoslide` + loop (declarative). _Done when:_ setting auto-advance writes `data-autoslide`/loop and the present route advances on the timer; byte-stable. (Spec 07, 10)

### Verification
- [x] **P17-21 — Tests + e2e.** Unit: serializer/sanitizer round-trip, range commands, link ops, chart contract, footer/number. Playwright: bold-a-word persists + survives reload; link add/remove; insert-chart renders (+ thumbnail placeholder); command palette runs a command; footer/number show; present-mode annotation loads offline; the offline-guard stays green. _Done when:_ all suites pass. (Spec 12)

> **Phase 17 STATUS (done, tag 0.0.18 — completes milestone M8):** Everyday-authoring parity. Built as **8 parallel git-worktree lanes** (A rich-text foundation; B+C formatting+links; D alt text; E command palette; F charts; G slide-numbers+footers; H present-mode) + a shared Go/contract foundation done by the orchestrator + integration-merge. Verified on the integrated tree: **vitest 1357** green (53 files), **svelte-check 0/0** (524 files), **go build/vet/test ./... green**, FE+binary build clean, binary smoke-tested (scaffold validates; present route injects chalkboard/laser + keeps deck.html byte-identical on disk; chart/chalkboard/laser vendored offline, zero external URLs).
> - **Foundation (orchestrator, `internal/deck/`):** vendored Chart.js 4.4.1 + a thin **self-authored** chart plugin (renders `<canvas data-chart data-chart-data='{json}'>` from a JSON attribute — cleaner for byte-stability/validation than the rajgoel CSV-in-innerHTML plugin), the rajgoel **chalkboard** plugin (+cursor images), and a **self-authored laser** plugin. Chart is in the scaffold template (`plugins:[…RevealChart]`); chalkboard/laser are vendored but NOT in the template.
> - **Lane A (`web/src/lib/model/inline.ts` + `canvas/writeback.ts`):** pure inline serializer + **sanitizer** over a fixed allowlist (`strong/em/u/s/a[href,target,rel]/span[style:color,font-size]/br`; b→strong, i→em, font→span; strips scripts/handlers/`javascript:`/external resource loads; localizes pasted images). `applyRichTextEdit(eid, html)` REPLACES the v1 flatten-to-text path (single writeback; the contenteditable commit now reads `innerHTML`). `classify` adds an `'inline'` class (marks never get a `data-eid`). validate.go needed **no change** (its tokenizer already balances inline tags). Golden corpus extended (`inline-marks.html`).
> - **Lane B+C (`canvas/inline-marks.ts`, `model/leaf-ops.ts`, `SelectionToolbar`/`LinkPopover`/`BlockTextControl`):** range wrap/unwrap mark toggles (B/I/U/S + font-size + colour-run, NOT execCommand) via a floating selection toolbar (tracks selection through zoom/pan, `preventDefault` keeps the range alive, commits through `applyRichTextEdit` = one undo); links (`linkRange`/`linkLeaf`, `isSafeHref`-gated, edit/remove via toolbar + text-leaf context menu); block-level text-align + list indent/outdent. Edit session re-enters the same leaf across the save-reload so formats chain.
> - **Lane D:** `applyAltText(eid, alt)` + `AltTextControl` (empty `alt=""` valid; provider description seeds alt).
> - **Lane E (`web/src/lib/commands/`):** pure `getCommands(ctx)` registry (22 commands, all dispatching existing `deckStore`/`selectionStore` methods — single source shared with menus); ⌘/Ctrl-K `CommandPalette` (fuzzy filter, keyboard nav, `when`-gated); `?` `ShortcutHelp` overlay. Guarded against text-editing focus.
> - **Lane F:** chart insert block + `ChartBlockPanel` (type + JSON editor) + `ChartDataControl` inspector + `applyChartData(eid,type,json)`; `data-chart`/`data-chart-data` recognized (classify leaf, layout.ts `getChartProps`, validate.go requires parseable JSON); script-free SVG **chart placeholder** in thumbnails (documented gap alongside code/KaTeX).
> - **Lane G:** deck-level slide-number toggle (`POST /api/decks/{name}/slide-number` → byte-stable `setSlideNumber` rewrite of `Reveal.initialize`); **CSS-based footer** (managed `/* slides-builder:footer */` block in custom.css via `customCssStore.setFooter/clearFooter`, rendered `section:not([data-footer-hidden])::after`) + per-slide `data-footer-hidden` opt-out (`setSlideFooterHidden`, validate-recognized).
> - **Lane H:** present-route plugin injection — `injectPresentPlugins` augments the IN-MEMORY present response with chalkboard+laser `<script>`s + `Reveal.registerPlugin(...)` (post-init registration; the vendored reveal 5.x inits a plugin registered after `initialize`), so **on-disk deck.html is byte-identical** and the editor canvas stays clean (annotations ephemeral, spec 10); auto-advance (`data-autoslide` per slide via `setSlideAutoslide`; deck-level `autoSlide`/`loop` via `POST /api/decks/{name}/autoslide` → `setRevealAutoSlide`). validate accepts `data-autoslide` as a non-negative int.
> - **Integration:** A/D/E/F/H merged with a few additive conflicts in shared files (`deck.svelte.ts`/`PropertiesPanel`/`classify.ts`/`model/index.ts`); B+C built correctly on the integrated Lane A; Lane G branched from an older base and was union-merged (duplicate `setInitKey`/`initLineRE` collapsed to one; two reveal-init rewrite helpers + two present handlers reconstructed as separate complete functions).
> - **⚠️ Needs browser confirmation:** `web/e2e/phase17.spec.ts` written + type-checks (inline marks + link render on present; chart renders via Chart.js; present injects plugins while deck.html stays byte-stable; slide-number shows; footer overlay renders + suppressed on `data-footer-hidden`; offline-guard) — run pending a Playwright browser env (`npm run test:e2e:docker`). Toolbar/palette/link-popover UX, chart inspector, footer/number live restyle need visual confirmation.
...
- **M8 (everyday-authoring parity):** Phase 17 — true inline text formatting + hyperlinks
  ...
  and present-mode drawing / laser / auto-advance. **Done (tag 0.0.18)** — 8 parallel worktree
  lanes + integration; vitest 1357 green, svelte-check 0/0, go green, binary smoke-tested. The
  Phase 18 per-slide-colour bug fix (tag 0.0.17) also landed. **Remaining across the whole plan:**
  P8-8 (MCP, deferred) and P11-3 (skip-reload-on-text-commit, deferred); browser-only visual/e2e
  confirmations noted per-phase (specs written, run pending a Playwright browser env).
