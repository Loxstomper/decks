...
**Phases 0–20 core are complete** (tags `0.0.1`–`0.0.18`; Phase 19, the QR block, and Phase 20
core, workspace resolution, landed on `main` untagged). That covers: the Go+Svelte skeleton and
...
polish; the QR block; and workspace resolution (the `slides` binary now runs from anywhere on
`$PATH`).
...
> Independent of workspace resolution (Phase 20), which is now done. Both make the resolved
> workspace fully pay off: one lets a deck be named by path, the other keeps the authoring skill
> in sync with the binary that validates it.
...
- [ ] **P21-2 — Binary ships the authoring skill.** `go:embed` `.claude/skills/slides-authoring/`
